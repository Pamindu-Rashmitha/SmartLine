    ...
    <div className="space-y-6">
        ...
        return (
          <div key={category.key} className="space-y-3">
            <div className="border-b border-slate-800 pb-2">
              <h2 className="text-base font-bold text-slate-200 m-0">{category.title}</h2>
              <p className="text-xs text-slate-400 mt-0.5 m-0">{category.description}</p>
            </div>
              ...
              {categoryConfigs.map(({ key, meta, data }) => {
                ...
                return (
                  <Col xs={24} sm={12} lg={8} key={key}>
                    <Card
                      className="bg-slate-900/80 border-slate-800 hover:border-slate-700 transition-all duration-300 rounded-xl shadow-lg relative group h-full flex flex-col justify-between"
                      bodyStyle={{ padding: '1.25rem', height: '100%', display: 'flex', flexDirection: 'column' }}
                    >
                      <div>
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex items-center gap-2">
                            <div className="p-2 rounded-lg bg-slate-800/80 text-blue-400 border border-slate-700/50">
                            ...
                            <div>
                              ...
                              <span className="text-[10px] font-mono text-slate-500">{data.configKey}</span>
                            </div>
                          </div>
                          ...
                          <Button
                            ...
                            className="bg-slate-800 hover:bg-slate-700 text-slate-300 border-slate-700 text-xs flex items-center"
                          >
                            Edit
                          </Button>
                        </div>
                      ...
                      <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-500">
                        ...
                        <span className="font-mono text-slate-400">by {data.updatedByUsername || 'SYSTEM'}</span>
                      </div>
                    </Card>
                  </Col>
                );
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSaveConfig} className="mt-4">
          <div className="p-3 mb-4 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300 flex items-start gap-2">
          ...
          <Form.Item
            label={<span className="text-slate-300 text-xs">Parameter Value</span>}
            name="configValue"
          ...
          <Form.Item
            label={<span className="text-slate-300 text-xs">Operational Description & Policy Note</span>}
            name="description"
          >
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-800">
            ...
            <Button type="primary" htmlType="submit" loading={saving} className="bg-blue-600 hover:bg-blue-500">
              Update Parameter
            </Button>
          </div>
        </Form>
